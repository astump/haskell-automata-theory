module HaskAut.FiniteAutomata.Regexp where

import HaskAut.Common.GvShow
import HaskAut.FiniteAutomata.NfaEpsilon
import HaskAut.FiniteAutomata.SubsetConstructionEps
import qualified HaskAut.FiniteAutomata.DfaOperations as Ops
import HaskAut.FiniteAutomata.Dfa(Dfa)
...
instance Show input => Show (Regexp input) where
  show Empty = "∅"
  show (Char c) = show c
