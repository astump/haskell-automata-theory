module HaskAut.Grammars.Pda where

import HaskAut.Common.GvShow
import Data.List
import HaskAut.Common.StackRelation
...
{- we don't have a bottom of stack symbol explicitly, but the code below
   assumes there is one, that appears at most once in any stack.  It
   also assumes that all stack symbols except the bottom of stack
   symbol represent a request to read at least one input character -}
...
toGraphViz :: (GvShow input, GvShow state, Eq state, Show stack) => Bool -> Pda input state stack -> String
toGraphViz printNodeNames (Pda states inputs _ trans eps start finals) =
    "digraph pda {\n" ++
    "rankdir = LR;\n" ++
...
-- show node names
writeGraphVizN :: (GvShow input, GvShow state, Eq state, Show stack) => String -> Pda input state stack -> IO ()
